<div class="col-md-10">
	<section class="content">
		...
		<div class="container">
			<table class="table">
				<tr>
					...
					<th <?php if ($this->session->userdata('role') === 'guest') echo 'class="d-none"' ?>>Action</th>
				</tr>
				<?php if ($jenis_perjanjian !== []) : ?>
					<?php foreach ($jenis_perjanjian as $jenisjanji) : ?>
						<tr>
							...
							<td>
								<a <?php if ($this->session->userdata('role') === 'guest') echo 'class="d-none"' ?> href="<?php echo base_url() ?>jenis_perjanjian/ubahjenisperjanjian/<?php echo $jenisjanji['id'] ?>" class="btn btn-warning"><i class="far fa-edit"></i></a>
								<a <?php if ($this->session->userdata('role') === 'guest') echo 'class="d-none"' ?> href="<?php echo base_url() ?>jenis_perjanjian/hapusjenisperjanjian/<?php echo $jenisjanji['id'] ?>" class="btn btn-danger" onclick="return confirm('apakah anda yakin?')"><i class="fas fa-trash"></i></a>
							</td>
						<?php endforeach; ?>
					<?php else : ?>
		...
		<!-- pemicu modal box -->
		<button <?php if ($this->session->userdata('role') === 'guest') echo 'class="d-none"' ?> type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">
